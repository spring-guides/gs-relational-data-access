...
an application and does a lot of the setup for you. This example needs the JDBC API and
H2 Database dependencies.

If you use Maven, visit the https://start.spring.io/#!type=maven-project&language=java&platformVersion=2.4.3.RELEASE&packaging=jar&jvmVersion=1.8&groupId=com.example&artifactId=relational-data-access&name=relational-data-access&description=Demo%20project%20for%20Spring%20Boot&packageName=com.example.relational-data-access&dependencies=jdbc,h2[Spring Initializr] to generate a new project with the required dependencies (JDBC API and H2 Database).
