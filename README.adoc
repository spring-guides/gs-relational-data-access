...
. Click *Generate*.
. Download the resulting ZIP file, which is an archive of a JDBC application that is configured with your choices.
...
first name/last name pairs in a Java array.
...
values when you make the call. The second argument is a Java 8 lambda that is used to
...
https://docs.spring.io/spring-boot/docs/current/reference/htmlsingle/#data.sql.datasource[Reference Guide]
for more detail.
